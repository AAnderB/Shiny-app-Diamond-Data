...
shinyServer(function(input, output) {
    ...
    output$distPlot <- renderPlot({
        ...
        data%>%ggplot(aes(x=volume,y=price,color=Carat_Category,alpha=input$alpha2))+
            geom_point(alpha=input$alpha2) + 
    ...
    output$distPlot2 <- renderPlot({
        data%>%ggplot(aes(x=Carat_Category,y=data$price))+geom_jitter(alpha=input$alpha1)+
